  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const res = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({ access_key: 'c4b7aadc-73a9-4932-884a-3876a0139512', name: ((data.firstName || '') + ' ' + (data.lastName || '')).trim() || 'Website Visitor', email: data.email, phone: data.phone || '', service: data.service || '', message: data.message }),
      });
      if (res.ok) {
        setSuccess(true);
        reset();
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: '48px', alignItems: 'start' }} className="contact-grid">
          ...
          >
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', paddingTop: '28px', marginTop: '8px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <a href="mailto:info@ntwebux.com" style={{ display: 'flex', alignItems: 'center', gap: '14px', textDecoration: 'none' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#93c5fd' }}>
                  <Mail size={16} />
                </div>
                <div>
                  ...
                  <div style={{ fontSize: '15px', fontWeight: 600, color: '#fff' }}>info@ntwebux.com</div>
                </div>
              </a>
            </div>
          </motion.div>
